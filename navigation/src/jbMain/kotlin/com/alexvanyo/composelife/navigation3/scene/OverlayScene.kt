 ...
 * popup window, etc.) that are visible above any additional [androidx.navigation3.scene.Scene] instances calculated from the
 * [overlaidEntries].
 *
 * When processing [overlaidEntries], expect processing of each [androidx.navigation3.scene.SceneStrategy] to restart from the
...
public interface OverlayScene<T : Any> : Scene<T> {
    ...
    /**
     * The [NavEntry]s that should be handled by another [Scene] that
