...
class PatternCollectionRepositoryImpl(
    ...
    private suspend fun synchronizePatternCollection(
        ...
        val archivePath = persistedDataPath /
            collectionsFolder /
            patternCollection.id.value.toString() /
            archiveFileName

        try {
            archivePath.parent?.let(fileSystem::createDirectories)
                ...
                .prepareGet(sourceUrl) {
                    onDownload { bytesSentTotal: Long, contentLength: Long? ->
                        ...
                        if (contentLength != null) {
                            logger.d { "progress: %.1f%".format(bytesSentTotal * 100f / contentLength) }
                        }
                    }
                }
                .execute { response ->
                    fileSystem.sink(archivePath).buffer().use { sink ->
                        response.bodyAsChannel().asSource().asOkioSource().buffer().use { source ->
                            // Use the real Dispatchers.IO here
                            // The handoff between the source and sink seems to block while awaiting, which locks up
                            // in tests if there is only one thread handling it
                            withContext(Dispatchers.IO) {
                                source.readAll(sink)
                            }
                        }
        ...
        // TODO: Compute a hash of the archive and skip extracting and doing more work if it is unchanged
...
private const val archiveFileName = "archive"
