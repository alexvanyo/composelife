...
interface GameOfLifeAlgorithm {
     ...
     */
    suspend fun computeGenerationWithStep(cellState: CellState, @IntRange(from = 0) step: Int): CellState

    fun computeGenerationsWithStep(originalCellState: CellState, @IntRange(from = 0) step: Int): Flow<CellState> =
        flow {
 ...
 */
suspend fun GameOfLifeAlgorithm.computeNextGeneration(cellState: CellState): CellState =
    computeGenerationWithStep(cellState = cellState, step = 1)
