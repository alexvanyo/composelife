...
val minuteSfdDirectory = layout.buildDirectory.dir("wff/minuteSfd")
tasks.withType<Task>().named { it == "jvmTest" }.configureEach {
    hourPrefixes.forEach { hourPrefix ->
        (0..59).forEach { minute ->
            outputs.file(
                minuteSfdDirectory.get().file("$hourPrefix:${"%02d".format(minute)}.sfd")
            )
        }
    }
}
...
androidComponents {
    hourPrefixes.forEach { hourPrefix ->
        val createHourSfd = tasks.register("createHour${hourPrefix}Sfd", CreateHourSfd::class) {
            ...
            dependsOn("jvmTest")
            minuteSfdInputDirectory = minuteSfdDirectory
