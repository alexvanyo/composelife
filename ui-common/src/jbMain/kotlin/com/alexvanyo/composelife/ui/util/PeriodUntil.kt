    ...
    produceState(
        ...
        while (isActive) {
            val targetInstant = this@periodUntil.plus(
                value.timeBasedDuration + unit.duration,
            )
            delay(targetInstant - clock.now())
            value = this@periodUntil.periodUntilWithUnit(
                other = clock.now(),
...
private fun Instant.periodUntilWithUnit(
    other: Instant,
    unit: DateTimeUnit,
    timeZone: TimeZone,
): DateTimePeriod {
            ...
            while (isActive) {
                ...
                val targetInstant = unitProgression.subList(0, unitIndex + 1).minOf { unit ->
                    this@progressivePeriodUntil.plus(
                        value.second.timeBasedDuration + unit.duration,
                    )
                }
                delay(targetInstant - clock.now())
                value = this@progressivePeriodUntil.periodUntilWithProgressiveUnits(
...
fun DateTimePeriod.inWholeUnits(unit: DateTimeUnit.DateBased): Int =
    when (unit) {
        is DateTimeUnit.DayBased -> days / unit.days
        is DateTimeUnit.MonthBased -> months / unit.months
    }

fun DateTimePeriod.inWholeUnits(unit: DateTimeUnit.TimeBased): Long =
    totalNanoseconds / unit.nanoseconds

private fun Instant.periodUntilWithProgressiveUnits(
    other: Instant,
    unitProgression: List<DateTimeUnit>,
    timeZone: TimeZone,
): Pair<DateTimeUnit, DateTimePeriod> {
    ...
    return periods.firstOrNull { (unit, period) ->
        when (unit) {
            is DateTimeUnit.TimeBased -> period.inWholeUnits(unit) > 0
            is DateTimeUnit.DateBased -> period.inWholeUnits(unit) > 0
        }
    } ?: periods.last()
}
...
private fun Instant.periodUntilWithProgressiveUnits(
    ...
    return periods.firstOrNull { (unit, period) ->
        abs(period.nanoseconds / unit.nanoseconds) > 0
    } ?: periods.last()
}
...
val DateTimePeriod.timeBasedDuration: Duration get() =
